...
void AnimationApp::Prepare()
{
  SetTitle("Animation VMD file");
  ...
  // モデルファイルをロード.
  const char* filePath = "初音ミク.pmd";  // モデルデータは各自用意してください。
  m_model.Prepare(this, filePath);
  m_model.SetShadowMap(m_shadowColor.shaderAccess);
  PrepareImGui();

  m_animator.Prepare("animation.vmd");  // アニメーションデータは各自用意してください。
...
void AnimationApp::OnMouseButtonDown(UINT msg)
{
  auto io = ImGui::GetIO();
...
void AnimationApp::OnMouseButtonUp(UINT msg)
{
  m_camera.OnMouseButtonUp();
}

void AnimationApp::OnMouseMove(UINT msg, int dx, int dy)
{
  auto io = ImGui::GetIO();
  if (io.WantCaptureMouse)
  {
...
void AnimationApp::PrepareShadowTargets()
{
...
void AnimationApp::PrepareImGui()
{
...
void AnimationApp::RenderToTexture()
{
...
void AnimationApp::RenderToMain()
{
...
void AnimationApp::UpdateImGui()
{
  ...
  ImGui::InputInt("Frame", (int*)&m_frameCount);
